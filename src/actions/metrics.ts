...
export async function getMetricsAction(opts: {
  ...
  // ── Datos base: clases activas con bookings en el rango ──
  ...
  // Calcular capacidad total en el rango: para cada día, sumar maxCapacity de clases ese día
  ...
  const totalBookingsAndCancelled = totalBookings + totalCancelled;
  const cancellationRate = totalBookingsAndCancelled > 0 ? Math.round((totalCancelled / totalBookingsAndCancelled) * 100) : 0;

  // Alumnos activos y en riesgo
  ...
  for (const c of classes) {
    ...
    const coachName = c.coach?.name ?? "Sin coach";
