...
function CustomTooltip({ active, payload, label, unit = "%" }: { active?: boolean; payload?: Array<{ value: number }>; label?: string; unit?: string }) {
  ...
  return (
    <div className="bg-[#0E2A38] border border-[#1A4A63] px-3 py-2 shadow-lg">
      ...
      <p className="text-sm font-bold text-[#EAEAEA] font-mono-data">{payload[0].value}{unit}</p>
    </div>
  );
}
    ...
    <div className="w-full" style={{ height }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart
          data={data}
          layout={layout}
          margin={{ top: 10, right: 10, left: isVertical ? 20 : -10, bottom: isVertical ? 0 : 40 }}
        >
          <defs>
          ...
          <Tooltip content={<CustomTooltip unit={unit} />} cursor={{ fill: "rgba(26, 74, 99, 0.2)" }} />
          <Bar
            dataKey="value"
