...
export function HeatmapCoachHour({ data }: Props) {
  const { hours, coaches, matrix } = useMemo(() => {
    ...
    for (const item of data) {
      ...
      if (!coachMap.has(item.coachId)) {
        coachMap.set(item.coachId, { id: item.coachId, name: item.coachName });
      }
    }
    ...
    for (const item of data) {
      matrix.set(`${item.hour}|${item.coachId}`, item);
    }

    return { hours, coaches, matrix };
  }, [data]);

  if (coaches.length === 0 || hours.length === 0) {
    return (
      ...
      <table className="w-full text-xs">
        <thead>
          <tr>
            ...
            {coaches.map((c) => (
              <th key={c.id} className="text-center text-[#6B8A99] font-medium uppercase tracking-wider py-2 px-1 min-w-[100px]">
