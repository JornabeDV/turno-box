"use client";

import { useState, useTransition } from "react";
import { toast } from "sonner";
import { adjustCreditsAction } from "@/actions/students";
import { cn } from "@/lib/utils";
import { DatePicker } from "@/components/ui/DatePicker";
import { ArrowLeftIcon, ArrowRightIcon } from "@phosphor-icons/react";

const MANUAL_METHODS = ["EFECTIVO", "TRANSFERENCIA", "TARJETA"];

type Props = { studentId: string; currentBalance: number };

const QUICK_AMOUNTS = [4, 8, 12, 16];

function formatARS(value: string, cursorPos: number | null) {
  const digits = value.replace(/\D/g, "");
  if (!digits) return { text: "", cursor: 0 };

  const num = Number(digits);
  const text = `$ ${new Intl.NumberFormat("es-AR").format(num)}`;

  // preservar posición del cursor basada en dígitos antes del cursor
  const beforeCursor = value.slice(0, cursorPos ?? 0);
  const digitsBefore = beforeCursor.replace(/\D/g, "").length;
  let newCursor = text.length;
  let count = 0;
  for (let i = 0; i < text.length; i++) {
    if (/\d/.test(text[i])) count++;
    if (count === digitsBefore) {
      newCursor = i + 1;
      break;
    }
  }
  return { text, cursor: newCursor };
}

export function AdjustCreditsForm({ studentId, currentBalance }: Props) {
  const [balance, setBalance] = useState(currentBalance);
  const [amount, setAmount] = useState("");
  const [amountPaid, setAmountPaid] = useState("");
  const [method, setMethod] = useState("EFECTIVO");
  const [validityDays, setValidityDays] = useState("30");
  const [validityMode, setValidityMode] = useState<"days" | "date">("days");
  const [expiresAt, setExpiresAt] = useState(() => {
    const d = new Date();
    d.setDate(d.getDate() + 30);
    return d.toISOString().slice(0, 10);
  });
  const [note, setNote] = useState("");
  const [error, setError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    const fd = new FormData();
    fd.set("amount", amount);
    fd.set("amountPaid", amountPaid.replace(/\D/g, ""));
    fd.set("method", method);
    fd.set("validityDays", validityDays);
    fd.set("validityMode", validityMode);
    fd.set("expiresAt", expiresAt);
    fd.set("note", note);

    startTransition(async () => {
      const res = await adjustCreditsAction(studentId, fd);
      if (res.success) {
        setBalance(res.data.newBalance);
        setAmount("");
        setAmountPaid("");
        setNote("");
        setValidityDays("30");
        setValidityMode("days");
        setExpiresAt(() => {
          const d = new Date();
          d.setDate(d.getDate() + 30);
          return d.toISOString().slice(0, 10);
        });
        const delta = Number(amount);
        toast.success(
          delta > 0
            ? `+${delta} créditos acreditados`
            : `${delta} créditos descontados`,
        );
      } else {
        setError(res.error);
        toast.error(res.error);
      }
    });
  }

  const preview = amount ? balance + Number(amount) : null;
  const isNegative = Number(amount) < 0;

  return (
    <div className="bg-card border border-border p-5">
      <div className="flex items-center justify-between">
        <h3 className="text-xs md:text-sm font-semibold text-secondary uppercase tracking-wider">
          Créditos disponibles
        </h3>
        <span
          className={cn(
            "text-2xl md:text-4xl font-black tabular-nums leading-none",
            balance === 0
              ? "text-danger"
              : balance <= 3
                ? "text-brand"
                : "text-success",
          )}
        >
          {balance}
        </span>
      </div>

      <form onSubmit={handleSubmit} className="space-y-3">
        {/* Atajos rápidos */}
        <div>
          <p className="text-[10px] md:text-sm text-muted uppercase tracking-wider mb-2 md:mb-3">
            Acreditar rápido
          </p>
          <div className="flex gap-2 md:gap-3">
            {QUICK_AMOUNTS.map((n) => (
              <button
                key={n}
                type="button"
                onClick={() => setAmount(String(n))}
                className={cn(
                  "flex-1 py-1.5 md:py-2.5 rounded-[2px] border text-xs md:text-sm font-bold transition-all active:scale-95",
                  amount === String(n)
                    ? "bg-brand/10 border-orange-500/40 text-brand"
                    : "border-border text-secondary hover:border-white/20",
                )}
              >
                +{n}
              </button>
            ))}
          </div>
        </div>

        {/* Input manual */}
        <div className="flex gap-2">
          <div className="flex-1">
            <input
              type="number"
              value={amount}
              onChange={(e) => setAmount(e.target.value)}
              placeholder="Monto (+8 o -2)"
              required
              className="w-full h-12 md:h-14 bg-page border border-border rounded-[2px] px-3 md:px-4 text-sm md:text-base text-primary placeholder:text-muted focus:outline-none focus:border-brand/50 transition-colors tabular-nums"
            />
          </div>
          <div className="flex-1 relative">
            <input
              type="text"
              inputMode="numeric"
              value={amountPaid}
              onChange={(e) => {
                const input = e.target;
                const start = input.selectionStart ?? 0;
                const { text, cursor } = formatARS(input.value, start);
                setAmountPaid(text);
                requestAnimationFrame(() => {
                  input.setSelectionRange(cursor, cursor);
                });
              }}
              placeholder="Monto pagado"
              className="w-full h-12 md:h-14 bg-page border border-border rounded-[2px] px-3 md:px-4 text-sm md:text-base text-primary placeholder:text-muted focus:outline-none focus:border-brand/50 transition-colors tabular-nums"
            />
          </div>
          {preview !== null && (
            <div
              className={cn(
                "h-12 md:h-14 px-3 md:px-4 rounded-[2px] border flex items-center gap-1 md:gap-1.5 text-xs md:text-sm font-bold tabular-nums shrink-0",
                isNegative
                  ? "bg-danger/10 border-danger/20 text-danger"
                  : "bg-success/10 border-success/20 text-success",
              )}
            >
              <ArrowRightIcon size={10} className="md:size-3" />
              {Math.max(0, preview)}
            </div>
          )}
        </div>

        {/* Método de pago (solo si hay monto pagado) */}
        {Number(amountPaid.replace(/\D/g, "")) > 0 && (
          <div>
            <p className="text-[10px] md:text-sm text-muted uppercase tracking-wider mb-1.5 md:mb-2">
              Método de pago
            </p>
            <div className="flex gap-2 md:gap-3">
              {MANUAL_METHODS.map((m) => (
                <button
                  key={m}
                  type="button"
                  onClick={() => setMethod(m)}
                  className={cn(
                    "flex-1 py-1.5 md:py-2.5 rounded-[2px] border text-xs md:text-sm font-medium transition-all active:scale-95",
                    method === m
                      ? "bg-brand/10 border-orange-500/40 text-brand"
                      : "border-border text-secondary hover:border-white/20",
                  )}
                >
                  {m.charAt(0) + m.slice(1).toLowerCase()}
                </button>
              ))}
            </div>
          </div>
        )}

        <p className="text-[10px] md:text-sm text-muted">Dejar en 0 o vacío si no hubo pago.</p>

        {/* Validez del abono */}
        <div>
          <p className="text-[10px] md:text-sm text-muted uppercase tracking-wider mb-1.5 md:mb-2">
            Validez del abono
          </p>
          <div className="flex gap-2 md:gap-3 mb-2 md:mb-3">
            <button
              type="button"
              onClick={() => setValidityMode("days")}
              className={cn(
                "flex-1 py-1.5 md:py-2.5 rounded-[2px] border text-xs md:text-sm font-medium transition-all active:scale-95",
                validityMode === "days"
                  ? "bg-brand/10 border-orange-500/40 text-brand"
                  : "border-border text-secondary hover:border-white/20",
              )}
            >
              Por días
            </button>
            <button
              type="button"
              onClick={() => setValidityMode("date")}
              className={cn(
                "flex-1 py-1.5 md:py-2.5 rounded-[2px] border text-xs md:text-sm font-medium transition-all active:scale-95",
                validityMode === "date"
                  ? "bg-brand/10 border-orange-500/40 text-brand"
                  : "border-border text-secondary hover:border-white/20",
              )}
            >
              Por fecha
            </button>
          </div>

          {validityMode === "days" ? (
            <div className="flex gap-2">
              {["7", "15", "30", "90"].map((d) => (
                <button
                  key={d}
                  type="button"
                  onClick={() => setValidityDays(d)}
                  className={cn(
                    "flex-1 py-1.5 md:py-2.5 rounded-[2px] border text-xs md:text-sm font-medium transition-all active:scale-95",
                    validityDays === d
                      ? "bg-brand/10 border-orange-500/40 text-brand"
                      : "border-border text-secondary hover:border-white/20",
                  )}
                >
                  {d} días
                </button>
              ))}
            </div>
          ) : (
            <div className="space-y-1.5">
              <DatePicker
                value={expiresAt}
                onChange={setExpiresAt}
                allowFuture
                showYearPicker={false}
                className="w-full [&_button]:md:h-14 [&_button]:md:text-base"
              />
              <p className="text-[10px] md:text-sm text-muted">
                Vence a las 23:59 hs del día seleccionado.
              </p>
            </div>
          )}
        </div>

        {/* Nota obligatoria */}
        <input
          type="text"
          value={note}
          onChange={(e) => setNote(e.target.value)}
          placeholder="Motivo (requerido)"
          required
          className="w-full h-12 md:h-14 bg-page border border-border rounded-[2px] px-3 md:px-4 text-sm md:text-base text-primary placeholder:text-muted focus:outline-none focus:border-brand/50 transition-colors"
        />

        {error && <p className="text-xs md:text-sm text-danger">{error}</p>}

        <button
          type="submit"
          disabled={isPending || !amount || !note}
          className="w-full uppercase h-12 md:h-14 rounded-[2px] bg-brand text-white text-xs md:text-base hover:bg-brand-hover active:scale-95 transition-all disabled:opacity-40"
        >
          {isPending ? (
            <span className="flex items-center justify-center gap-2">
              <span className="size-3.5 rounded-full border-2 border-white border-t-transparent animate-spin" />
              Aplicando…
            </span>
          ) : (
            `Cargar créditos`
          )}
        </button>
      </form>
    </div>
  );
}
