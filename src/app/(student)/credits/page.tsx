  ...
  return (
    <section className="pt-4 md:pt-8 space-y-5 md:space-y-8">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="font-[family-name:var(--font-oswald)] font-bold text-primary uppercase tracking-tight text-2xl md:text-4xl">
            Mis créditos
          </h1>
        ...
        <Link
          href="/packs"
          className="inline-flex items-center gap-1.5 px-3 py-2 md:px-5 md:py-3 bg-brand text-page text-xs md:text-sm font-[family-name:var(--font-oswald)] font-bold uppercase tracking-wide active:scale-[0.98] transition-transform"
        >
          Comprar
        </Link>
      </div>
      ...
      <div className="bg-card border border-border p-5 md:p-8">
        <div className="flex items-center gap-4 md:gap-6">
          <div className="size-14 md:size-20 border border-brand/30 bg-brand/10 flex items-center justify-center shrink-0">
            <Wallet size={24} weight="bold" className="text-brand md:size-8" />
          </div>
          <div>
            <p className="text-[10px] md:text-xs font-[family-name:var(--font-jetbrains)] uppercase tracking-wider text-secondary">
              Disponibles
            </p>
            <p className="font-[family-name:var(--font-oswald)] font-bold text-primary text-3xl md:text-4xl leading-none mt-0.5 md:mt-1">
              {totalCredits}{" "}
              <span className="text-lg md:text-2xl text-secondary">
        ...
        <div className="flex items-center gap-6 md:gap-10 mt-4 md:mt-6 pt-4 md:pt-6 border-t border-border">
          <div>
            <p className="text-[10px] md:text-xs font-[family-name:var(--font-jetbrains)] uppercase tracking-wider text-muted">
              Abonos activos
            </p>
            <p className="text-sm md:text-lg font-[family-name:var(--font-oswald)] font-bold text-primary mt-0.5 md:mt-1">
              {activeSubs.length}
            </p>
          </div>
          {nextExpiry !== null && (
            <div>
              ...
              <p
                className={cn(
                  ...
                  nextExpiry <= 7 ? "text-brand" : "text-primary",
                )}
              >
      ...
      {activeSubs.length > 0 && (
        <div className="space-y-3 md:space-y-4">
          <h2 className="font-[family-name:var(--font-oswald)] font-bold text-primary uppercase tracking-tight text-lg md:text-2xl">
            Abonos activos
          </h2>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3 md:gap-4">
                    <div>
                      <p className="text-sm md:text-lg font-[family-name:var(--font-oswald)] font-bold text-primary uppercase tracking-tight">
                        {sub.pack?.name ?? "Abono"}
                      </p>
                      <p className="text-[11px] md:text-sm text-muted mt-0.5 md:mt-1 font-[family-name:var(--font-jetbrains)]">
                        Comprado{" "}
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-sm md:text-xl font-[family-name:var(--font-oswald)] font-bold text-success">
                        {sub.remaining}
                        <span className="text-secondary text-xs md:text-sm">
                          /{sub.creditsGranted}
                        </span>
                      </p>
                      <p className="text-[10px] md:text-xs text-muted font-[family-name:var(--font-jetbrains)]">
                        restantes
                      </p>
                    </div>
                  </div>
                  ...
                  {/* Barra de progreso */}
                  <div className="mt-3 md:mt-4 h-1 md:h-1.5 bg-border overflow-hidden">
                    <div
                      className={cn(
                        "h-full transition-all",
                        sub.isExpiringSoon ? "bg-brand" : "bg-success",
                      )}
                  ...
                  <div className="flex items-center justify-between mt-2 md:mt-3">
                    <p className="text-[10px] md:text-xs font-[family-name:var(--font-jetbrains)] text-muted uppercase tracking-wider">
                      {sub.used} usadas
                    </p>
                    {sub.expiresAt && (
                      <p
      ...
      <div className="space-y-3 md:space-y-4">
        ...
        {visibleTransactions.length === 0 ? (
          <div className="bg-card border border-border px-4 py-12 md:py-16 text-center">
            <Clock size={24} className="text-border mx-auto mb-2 md:mb-3 md:size-8" />
                  ...
                  >
                    {isPositive ? (
                      <Plus
                        size={14}
                        weight="bold"
                        className="text-success md:size-5"
                      />
                    ) : (
                      <Minus
                        size={14}
                        weight="bold"
                        className="text-danger md:size-5"
                      />
                    )}
                  </div>
