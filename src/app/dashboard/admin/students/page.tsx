  ...
  const students = await prisma.user.findMany({
    ...
    select: {
      ...
      _count: {
        select: {
          bookings: {
            where: { status: "CONFIRMED", classDate: { gte: today }, deletedAt: null },
          },
        },
      },
    },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h2 className="text-xl font-bold text-zinc-100 tracking-tight">Alumnos</h2>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <MetricCard label="Con turno hoy" value={withBookings} icon="calendar" accent="orange" />
      </div>

      {inactive > 0 && (
        <p className="text-xs text-zinc-600 px-1">
          {inactive} {inactive === 1 ? "alumno inactivo" : "alumnos inactivos"} — no aparecen en reservas
        </p>
      )}
